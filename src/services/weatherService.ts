...
export const fetchWeather = async (lat: number, lon: number): Promise<WeatherData> => {
    try {
        const response = await fetch(
            `https://api.open-meteo.com/v1/forecast?latitude=${lat}&longitude=${lon}&current=temperature_2m,relative_humidity_2m,apparent_temperature,is_day,precipitation,weather_code,wind_speed_10m&hourly=temperature_2m&timezone=auto`
        );
        ...
        if (current.temperature_2m > 30) insight = "High Temp: Check Ice levels & Cold Bev inventory.";
        if (current.temperature_2m < 5) insight = "Cold: Prepare Coffee & ensure windshield fluid availability.";

    ...
    <div class="flex flex-col md:flex-row md:items-end justify-between gap-8 animate-in fade-in slide-in-from-top-4 duration-700">
      <div>
        <div class="flex items-center gap-3 mb-4">
          ...
          <div>
            <span class="text-xs font-black text-slate-400 uppercase tracking-[0.4em]">Terminal Operations</span>
      ...
      <div class="flex gap-4">
        ...
        <button @click="triggerScan" 
          ...
          <div class="p-2 bg-indigo-50 text-indigo-600 rounded-xl group-hover:scale-110 transition-transform">
           ...
           <div class="text-left">
            <p class="text-xs font-black text-slate-400 uppercase tracking-widest mb-0.5">Automated Flow</p>
            <p class="text-sm font-black text-slate-900 leading-none">Scan Manifest BOL</p>
          </div>
        </button>

        <div class="bg-white px-8 py-5 rounded-3xl border border-slate-100 shadow-sm flex items-center gap-6">
          <div class="text-right">
            <p class="text-xs font-black text-slate-400 uppercase tracking-widest mb-1">Total Payload</p>
            <p class="text-2xl font-black text-slate-900 tracking-tighter">{{ totals.totalGallons.toLocaleString() }} <span class="text-base opacity-40">GAL</span></p>
          </div>
          <div v-if="totals.totalAmount > 0" class="flex items-center gap-6">
            <div class="w-px h-10 bg-slate-100"></div>
            <div class="text-right">
              <p class="text-xs font-black text-slate-400 uppercase tracking-widest mb-1">Total Value</p>
        ...
        <div class="bg-white rounded-[3rem] p-10 shadow-sm border border-slate-100 relative overflow-hidden">
          ...
          <div class="relative z-10">
            <div class="grid grid-cols-1 md:grid-cols-3 gap-8 mb-10 pb-10 border-b border-slate-50 text-slate-900">
              <div class="space-y-3">
                <label class="text-xs font-black text-slate-400 uppercase tracking-widest flex items-center gap-2 ml-2">
            ...
            <div class="space-y-6">
              <div class="flex items-center justify-between mb-4">
                ...
                <button @click="addItem" class="flex items-center gap-2 px-4 py-2 bg-indigo-50 text-indigo-600 rounded-xl text-xs font-black uppercase tracking-widest hover:bg-indigo-100 transition-all">
              ...
              <div v-for="(item, index) in items" :key="index" 
                ...
                <div class="flex flex-col gap-2">
                  <label class="text-xs font-black text-slate-300 uppercase tracking-widest ml-1">Dump / Type</label>
                  <select v-model="item.type" class="bg-white border-none rounded-xl px-4 py-3 text-sm font-bold focus:ring-2 ring-indigo-500 shadow-sm text-slate-900">
                    <option v-for="type in fuelStore.defaultFuelTypes" :key="type">{{ type }}</option>
                  </select>
                </div>

                <div class="flex flex-col gap-2">
                  <label class="text-xs font-black text-slate-300 uppercase tracking-widest ml-1">Gallons</label>
                  <input type="number" v-model.number="item.gallons" class="bg-white border-none rounded-xl px-4 py-3 text-sm font-black focus:ring-2 ring-indigo-500 shadow-sm text-slate-900" placeholder="0" />
                </div>

                <div class="flex flex-col gap-2">
                  <label class="text-xs font-black text-slate-300 uppercase tracking-widest ml-1">Cost / Gal</label>
                  <div class="relative">
                    <span class="absolute left-3 top-1/2 -translate-y-1/2 text-slate-300 text-sm">$</span>
                    <input type="number" step="0.0001" v-model.number="item.costPerGal" class="w-full bg-white border-none rounded-xl pl-6 pr-4 py-3 text-sm font-black focus:ring-2 ring-indigo-500 shadow-sm text-slate-900" placeholder="0.0000" />
                  </div>
                </div>

                <div class="flex flex-col gap-2">
                  <label class="text-xs font-black text-slate-300 uppercase tracking-widest ml-1">Retail / Gal</label>
                  <div class="relative">
                    <span class="absolute left-3 top-1/2 -translate-y-1/2 text-slate-300 text-sm">$</span>
                    <input type="number" step="0.001" v-model.number="item.retailPerGal" class="w-full bg-white border-none rounded-xl pl-6 pr-4 py-3 text-sm font-black focus:ring-2 ring-indigo-500 shadow-sm text-slate-900" placeholder="0.000" />
                  </div>
                </div>
                ...
                <div v-if="item.gallons > 0 && item.costPerGal > 0" class="col-span-full mt-4 flex items-center gap-6 px-4 py-3 bg-white/50 rounded-2xl border border-slate-50 animate-in fade-in zoom-in duration-300">
                  <div class="flex items-center gap-2">
                    ...
                    <span class="text-xs font-black text-slate-400 uppercase tracking-widest">Margin:</span>
                    <span class="text-sm font-black text-emerald-600">{{ (calculatedItems[index]?.profitMargin || 0).toFixed(1) }}%</span>
                  </div>
                  <div class="flex items-center gap-2 border-l border-slate-100 pl-6">
                    <BarChart3 class="w-3.5 h-3.5 text-indigo-500" />
                    <span class="text-xs font-black text-slate-400 uppercase tracking-widest">Load Profit:</span>
                    <span class="text-sm font-black text-indigo-600">${{ (calculatedItems[index]?.totalProfit || 0).toLocaleString(undefined, { maximumFractionDigits: 0 }) }}</span>
                  </div>
                  <div class="flex items-center gap-2 border-l border-slate-100 pl-6">
                    <DollarSign class="w-3.5 h-3.5 text-slate-400" />
                    <span class="text-xs font-black text-slate-400 uppercase tracking-widest">Fuel Cost:</span>
                    <span class="text-sm font-black text-slate-900">${{ (calculatedItems[index]?.totalCost || 0).toLocaleString(undefined, { maximumFractionDigits: 0 }) }}</span>
                  </div>
                </div>
              </div>
            </div>
      ...
      <div class="lg:col-span-4 space-y-8 animate-in fade-in slide-in-from-right-4 duration-700 delay-200">
        ...
        <div class="bg-gradient-to-br from-indigo-900 to-slate-900 rounded-[3rem] p-8 text-white shadow-2xl relative overflow-hidden">
          ...
          <div class="relative z-10">
            <div class="flex items-center gap-3 mb-6">
              ...
              <h3 class="text-sm font-black uppercase tracking-widest text-indigo-300">Delivery Yield Pool</h3>
            </div>
            <p class="text-xs font-black text-white/50 uppercase tracking-[0.2em] mb-1">Maneuverable Profit recorded</p>
            ...
            <p class="text-sm font-bold text-indigo-200/60 leading-relaxed">
        ...
        <div class="bg-white rounded-[3rem] p-8 border border-slate-100 shadow-sm overflow-hidden">
          <div class="flex items-center justify-between mb-8">
            ...
            <span class="text-xs font-black text-slate-400 uppercase tracking-widest">Master Logs</span>
          </div>
            ...
            <div v-for="delivery in fuelStore.deliveries" :key="delivery.id" class="p-6 bg-slate-50 rounded-[2.5rem] border border-slate-50 hover:bg-white hover:shadow-xl transition-all group">
              <div class="flex items-center justify-between mb-4 pb-4 border-b border-slate-100">
                <div class="flex items-center gap-4">
                  ...
                  <div>
                    <p class="text-sm font-black text-slate-900 uppercase italic leading-none truncate w-40">{{ delivery.supplier }}</p>
                    <p class="text-xs font-black text-slate-400 mt-1 uppercase tracking-widest">Manifest ID: {{ delivery.id.substring(0,8) }}</p>
                  </div>
                </div>
                <div class="text-right">
                  <p class="text-xs font-black text-slate-400 uppercase tracking-widest mb-1">Total Cost</p>
              ...
              <div class="grid grid-cols-2 md:grid-cols-4 gap-4 mb-4">
                <div class="space-y-1">
                  <p class="text-xs font-black text-slate-400 uppercase tracking-widest">Delivery Date</p>
                  <p class="text-sm font-bold text-slate-700">{{ formatDate(delivery.date) }}</p>
                </div>
                <div class="space-y-1">
                  <p class="text-xs font-black text-slate-400 uppercase tracking-widest">EFT Draft Date</p>
                  <p class="text-sm font-bold text-indigo-600">{{ delivery.eftDate ? formatDate(delivery.eftDate) : 'N/A' }}</p>
                </div>
                <div class="space-y-1">
                  <p class="text-xs font-black text-slate-400 uppercase tracking-widest">Total Volume</p>
                  <p class="text-sm font-bold text-slate-700">{{ delivery.totalGallons.toLocaleString() }} GAL</p>
                </div>
                <div class="space-y-1 text-right">
                  <p class="text-xs font-black text-slate-400 uppercase tracking-widest">Retail Profit</p>
                  <p class="text-sm font-black text-emerald-600">${{ delivery.totalProfit.toLocaleString(undefined, { maximumFractionDigits: 0 }) }}</p>
                </div>
              </div>
               ...
               <div class="pt-4 border-t border-slate-50">
                <div class="flex flex-wrap gap-2">
                   <div v-for="item in delivery.items" :key="item.type" class="px-3 py-1 bg-white rounded-lg border border-slate-100 flex items-center gap-2">
                     <span class="text-xs font-black text-slate-400 uppercase">{{ item.type }}:</span>
                     <span class="text-sm font-black text-slate-700">${{ item.retailPerGal.toFixed(3) }}</span>
                     <span class="text-xs font-bold text-emerald-500 italic">({{ item.profitMargin.toFixed(1) }}%)</span>
                   </div>
                </div>
              </div>
            </div>

            ...
            <div class="flex flex-wrap items-center gap-3">
                 <div v-if="activeTab === 'Schedule'" class="flex items-center gap-3 mr-2">
                     ...
                     <div class="flex items-center gap-2 px-3 py-1.5 bg-white rounded-xl border border-slate-100 shadow-sm">
                        ...
                        <span class="text-xs font-black text-emerald-700 uppercase tracking-tighter">${{ estimatedWeeklyCost.toFixed(0) }}</span>
                     </div>
                 </div>
                 ...
                 <div class="flex items-center bg-slate-100/50 rounded-xl p-1">
                    ...
                    <div class="px-4 flex items-center gap-2">
                        ...
                        <span class="text-xs font-black text-slate-900 uppercase tracking-widest min-w-[120px] text-center">
                            {{ formatDateRange }}
                        </span>
                    </div>
                     <button @click="navigateWeek(1)" class="p-2 hover:bg-white rounded-lg text-slate-400 hover:text-slate-900 transition-all">
                    ...
                    <thead>
                        <tr>
                            <th class="p-3 bg-slate-50/50 border-b border-r border-slate-100 w-[180px] sticky left-0 z-20">
                                <span class="text-xs font-black text-slate-400 uppercase tracking-[0.2em]">Employee</span>
                            </th>
                            <th v-for="day in weekDays" :key="day.toISOString()" class="p-2 bg-slate-50/50 border-b border-slate-100 text-center">
                                <div class="flex flex-col items-center">
                                    <div class="text-xs font-black text-slate-400 uppercase tracking-widest">{{ day.toLocaleDateString('en-US', {weekday: 'short'}) }}</div>
                        ...
                        <tr v-for="emp in employeeStore.employees" :key="emp.id" class="group hover:bg-slate-50/30 transition-colors">
                            <td class="p-3 border-r border-slate-100 sticky left-0 bg-white group-hover:bg-slate-50 transition-colors z-10 shadow-[2px_0_5px_-3px_rgba(0,0,0,0.05)]">
                                <div class="flex items-center gap-3">
                                    ...
                                    <div class="min-w-0">
                                        <div class="font-black text-slate-900 text-sm truncate leading-tight">{{ emp.firstName }} {{ emp.lastName }}</div>
                                        <div class="text-[10px] font-bold text-slate-400 uppercase tracking-widest truncate">{{ emp.position }}</div>
                                    </div>
                                </div>
                            </td>
                                    ...
                                    >
                                        ...
                                        <div class="pl-1.5 space-y-0.5">
                                            <div class="flex items-center justify-between">
                                                 <span :class="`text-[10px] font-black uppercase tracking-tight text-${getRoleColor(shift.role)}-700`">{{ shift.role }}</span>
                                                ...
                                                <div class="flex justify-between items-center">
                                                    <span class="text-[10px] font-bold text-slate-400 uppercase">{{ getShiftDuration(shift).toFixed(0) }}h</span>
            ...
            <div v-else class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                <div v-for="req in shiftStore.timeOffRequests" :key="req.id" class="p-6 rounded-3xl border border-slate-100 bg-slate-50/30">
                    <div class="flex items-center gap-3 mb-4">
                        ...
                        <div>
                            <div class="font-black text-slate-900 text-base">{{ req.employeeName }}</div>
                            <div class="text-xs font-black uppercase tracking-widest" :class="{
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                <div v-for="swap in shiftStore.swapRequests" :key="swap.id" class="glass-panel p-6 border-slate-100 bg-white">
                    <div class="flex justify-between items-start mb-4">
                        <span 
                            class="px-2 py-1 rounded-lg text-xs font-black uppercase tracking-widest"
                            :class="{
                        ...
                        <span class="text-xs text-slate-400 font-bold">{{ swap.createdAt.toDate().toLocaleDateString() }}</span>
                    </div>

                    <div class="space-y-4">
                        <div>
                            <p class="text-xs font-black text-slate-400 uppercase tracking-widest mb-1">Requester</p>
                            <p class="font-black text-slate-900 text-lg">{{ getEmployeeName(swap.requesterId) }}</p>
                        </div>
                        <div>
                            <p class="text-xs font-black text-slate-400 uppercase tracking-widest mb-1">Target Shift</p>
                            <p class="text-sm font-bold text-slate-700">{{ getShiftDetails(swap.targetShiftId) }}</p>
                        </div>
                        <div>
                            <p class="text-xs font-black text-slate-400 uppercase tracking-widest mb-1">Proposed Recipient</p>
                            <p class="font-black text-lg" :class="swap.recipientId ? 'text-slate-900' : 'text-primary-600 italic'">
                    ...
                    <div v-if="swap.status === 'Pending'" class="flex gap-2 mt-8">
                        <button @click="handleApproveSwap(swap)" class="btn-primary flex-1 py-2 text-xs">Approve</button>
                        <button @click="handleRejectSwap(swap.id)" class="btn-secondary flex-1 py-2 text-xs">Reject</button>
                    </div>
                </div>
            ...
            <div class="overflow-hidden border border-slate-100 rounded-2xl">
                <table class="w-full text-left">
                    <thead class="bg-slate-50">
                        <tr>
                            <th class="p-6 text-xs font-black uppercase tracking-widest text-slate-400">Employee</th>
                            <th class="p-6 text-xs font-black uppercase tracking-widest text-slate-400">Sch. Hrs</th>
                            <th class="p-6 text-xs font-black uppercase tracking-widest text-slate-400">Act. Hrs</th>
                            <th class="p-6 text-xs font-black uppercase tracking-widest text-slate-400">Hr. Variance</th>
                            <th class="p-6 text-xs font-black uppercase tracking-widest text-slate-400">Cost Var.</th>
                            <th class="p-6 text-xs font-black uppercase tracking-widest text-slate-400">Punctuality</th>
                            ...
                            <td class="p-6">
                             ...
                             <td class="p-6">
                                <div class="flex items-center gap-2">
                                    ...
                                    <span class="text-xs font-black text-slate-900">{{ item.punctuality.toFixed(0) }}%</span>
                                </div>
                            </td>
                            <td class="p-6">
                                <span 
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                <div v-for="emp in employeeStore.employees" :key="emp.id" class="p-6 rounded-3xl border border-slate-100 bg-white shadow-sm hover:shadow-md transition-all">
                    <div class="flex items-center gap-4 mb-6">
                        ...
                        <div>
                            <h4 class="font-black text-slate-900 leading-none text-base">{{ emp.firstName }} {{ emp.lastName }}</h4>
                            <p class="text-xs font-black text-primary-500 uppercase tracking-widest mt-1">{{ emp.position }}</p>
                        </div>
                    </div>
                    ...
                    <button @click="openEditAvailability(emp.id)" class="w-full mt-6 py-2 text-xs font-black uppercase tracking-widest text-slate-400 border border-dashed border-slate-200 rounded-xl hover:border-primary-200 hover:text-primary-500 transition-all">
                        Edit Pattern
                    </button>
                </div>
            </div>
            ...
            <div class="glass-panel w-full max-w-2xl relative z-10 bg-white p-0 overflow-hidden shadow-2xl rounded-3xl scale-in-center">
                <div class="bg-primary-600 p-6 flex items-center justify-between text-white">
                    <div>
                        ...
                        <p class="text-xs text-primary-100 font-black uppercase tracking-widest mt-1">
